...
5. **On verify red:** the change is already on `main`. First **triage the owner** (see Cross-repo routing): an Solaris-owned regression → identify the culprit (a cluster keeps it attributable; an unrelated batch needs a bisect), open a **revert PR**, merge it on CI-green, re-run this verify; write `verify-result.json` `status:"red"` so the orchestrator holds the release.
   **The working tree is shared with a concurrent builder.** Any revert or hotfix branch you create starts from `origin/main` explicitly — `git fetch origin && git checkout -B <branch> origin/main` — never from whatever branch the tree happens to be on (on 2026-09-06 a hotfix branch cut from the builder's `batch/…` branch carried two unfinished foreign commits into PR #1351). Keep the branch to your own commit(s), confirm the PR lists exactly those, and `git checkout main` right after pushing so the builder finds the tree where it left it.
   **Revert commits use git's canonical form** — subject exactly `Revert "<the reverted commit's subject, verbatim>"` plus a `This reverts commit <full-sha>.` body line (i.e. let `git revert` write it; don't hand-author a `revert(scope): …` subject). Two reasons: the canonical form is what conventional-commit revert detection matches, and a hand-written subject tends to carry a `(#1221)` PR-number token, which violates the paren rule in `CLAUDE.md`. **Then check the release-please PR**: whether it actually drops the reverted entry is *not* guaranteed — on 2026-08-25 a `revert(template): …` subject left the reverted fix listed as delivered *and* added a Reverts section, so 0.40.0 claimed a fix that was no longer in the tree. If the reverted entry survives in `CHANGELOG.md`, say so in `verify-result.json` `detail` so the orchestrator can surface it; the release PR is a human's to correct. A **ServiceBay-platform** failure → do **not** revert a correct Solaris change: file it upstream (`gh issue create --repo mdopp/servicebay`, symptom + servicebay file/line + the Solaris repro), note it in the result `detail`, and write `status:"owed"` (the planner will mark the local issue blocked + `upstream_waits[]` next run; release stays held until the upstream fix lands and re-verify is green).
