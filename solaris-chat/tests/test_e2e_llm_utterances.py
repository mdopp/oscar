...
async def test_e2e_llm_1live_resolution(tmp_path):
    ...
    messages = [
        ...
        {"role": "user", "content": "Spiele den Radiosender 1live im Wohnzimmer"},
    ]
    ...
    assert t_name in ("play_radio", "play_music"), (
        f"Unexpected tool generated: {t_name}"
    )
    ...
    assert str(out.get("station")).lower() in ("1live", "1 live"), (
        f"Unexpected station: {out}"
    )
