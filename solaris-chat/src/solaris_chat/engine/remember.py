...
def wants_remember(text: str) -> str | None:
    ...
    The trigger is anchored at the start, and a real turn never starts with the
    resident's word: the proxy prepends `[Aktuelle Zeit: …]` and the gatekeeper
    `[room: …]`. Those leading blocks come off first — the bench sentence and
    the turn a resident actually sends must reach the same verdict (#1336)."""
    m = _TRIGGER_RE.match(strip_internal_hints(text or ""))
    if not m:
        return None
