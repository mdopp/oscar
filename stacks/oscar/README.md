...
1. ServiceBay's `ai-stack` — Postgres, Ollama, (optional Qdrant), Hermes
2. OSCAR's `oscar-household` template — the household-specific glue
...
1. **`postgres`** — pick a Postgres password (auto-generated by the wizard), pick a data volume. This Postgres is shared by Hermes (Honcho) and OSCAR (audit + Phase-3a domain memory).
2. **`ollama`** — choose model (`gemma-12b-q4` by default), enable GPU passthrough if you have a CDI-registered NVIDIA GPU. First start pulls the model (5–10 min).
3. **`qdrant`** (optional, Phase 3a) — keep disabled until you start the ingestion pipeline.
4. **`hermes`** — wraps `docker.io/nousresearch/hermes-agent`. Wizard prompts: `API_SERVER_KEY` (auto-generated), `LLM_PROVIDER_URL` (defaults to the `ollama` template's Ollama port), Postgres DSN (from step 1).
...
- `POSTGRES_DSN` — same Postgres deployed in Step 1
...
- Runs Alembic against the shared Postgres, creating `cloud_audit`, `system_settings`, `voice_embeddings` (idempotent)
- Bind-mounts OSCAR's `skills/` directory into Hermes at `/opt/data/skills/oscar`
...
If anything answers "nein", run `oscar-status` first — it returns a structured probe of Postgres, Ollama, Hermes, HA-MCP, ServiceBay-MCP.
