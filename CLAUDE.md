...
- **Voice ↔ uid is OSCAR's job, not Hermes'.** The gatekeeper does speaker embedding + LLDAP-uid lookup + passes uid as a request parameter to Hermes. Voice embeddings live in OSCAR's Postgres, **never** in LLDAP — biometric PII.
...
└── oscar-household/          # the one OSCAR ServiceBay template
                              #   - runs Alembic against the shared Postgres
...
| **Postgres, Ollama, Qdrant, Hermes** | `ai-stack` (planned in `mdopp/servicebay`) |
...
3. *Phase 0:* `uid = DEFAULT_UID`. *Phase 2:* SpeechBrain ECAPA-TDNN extracts a 256-d voice embedding; lookup against `voice_embeddings` in Postgres resolves to an LLDAP `uid`.
...
Two memory layers, both `uid`-namespaced via the request parameter the gatekeeper passes per turn:

- **Hermes (Honcho + FTS5)**: conversation history, skill curation. Persisted under the Hermes container's data volume.
- **OSCAR Postgres (+ Qdrant in Phase 3a)**: audit + domain memory. Lives in the shared `ai-stack` Postgres.

Three Phase 0–2 tables: `cloud_audit`, `system_settings`, `voice_embeddings`. Phase 3a adds the domain collections (`books`, `records`, `documents`, `audiobooks`, `experiences`).

Voice embeddings are **never** in LLDAP. Biometric PII goes in OSCAR's Postgres only.
...
- **Logging.** Operational logs → container stdout JSON → journald → ServiceBay-MCP (`get_container_logs`). Domain audit → Postgres → `oscar-audit-query`. Conversation logs → Hermes-native. `trace_id` correlates the three.

## Phase plan (digest)

- **Phase 0 — Chat on Hermes + lights.** Prereqs: ServiceBay v3.16+ with full-stack; `mdopp/servicebay#443` merged (registry sync); the new ServiceBay `ai-stack` templates (`postgres`, `ollama`, `hermes`). Deploy `ai-stack` + `oscar-household`. Pair Signal via `hermes gateway setup signal`. Add HA-MCP via `hermes mcp add`. First household skill: `smart-home/home-assistant` (upstreamed to Hermes Skills Hub, consumed via `hermes skill add`).
...
- `mdopp/servicebay`: `postgres`, `qdrant`, `ollama`, `hermes` templates; `ai-stack` walkthrough; `voice` template extension with `GATEKEEPER_IMAGE`; structured-logging + health-probe contracts
